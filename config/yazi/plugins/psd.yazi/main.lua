...
function M:preload(job)
  ...
  local cmd = M.with_limit():arg(tostring(job.file.path) .. "[0]")
  if job.args.flatten then cmd:arg("-flatten") end
  ...
  if job.args.bg then cmd:arg({ "-background", job.args.bg, "-alpha", "remove" }) end

  local status, err = cmd:arg(string.format("JPG:%s", cache)):status()
  if not status then
    return true, Err("Failed to start `magick`, error: %s", err)
  elseif not status.success then
    return false, Err("`magick` exited with error code: %s, error: %s", status.code, err)
  else
    return true
  end
end
...
function M.with_limit()
  ...
  if rt.tasks.image_bound[1] > 0 then cmd:arg({ "-limit", "width", rt.tasks.image_bound[1] }) end
  if rt.tasks.image_bound[2] > 0 then cmd:arg({ "-limit", "height", rt.tasks.image_bound[2] }) end
  return cmd
end
